...
- (void)viewDidLoad
{
    ...
    titleLab.text=@"万柳桥麻将机";
    titleLab.textColor=[UIColor whiteColor];
    [infoView addSubview:titleLab];
    //添加市场价格
    addressLab=[[UILabel alloc] initWithFrame:CGRectMake(15, 40, 300, 50)];
    addressLab.backgroundColor=[UIColor clearColor];
    addressLab.numberOfLines=0;
    addressLab.font=[UIFont fontWithName:@"ZhunYuan" size:17];
    addressLab.text=@"地址：成都市成华区万年场\r电话：028-757656543";
    addressLab.textColor=[UIColor whiteColor];
    [infoView addSubview:addressLab];
    //购买按钮
    UIImage *navImage=[UIImage imageNamed:@"bt_daohang"];
    UIButton *navButton=[[UIButton alloc] initWithFrame:CGRectMake(20, 90, 100, 100*navImage.size.height/navImage.size.width)];
    ...
    //添加分割线
    UIImageView *lineImageView=[[UIImageView alloc] initWithFrame:CGRectMake(10, 130, 300, 2)];
    lineImageView.image=[UIImage imageNamed:@"linedown_xiangqing"];
    [infoView addSubview: lineImageView];
    //添加详细说明
    detailTextView=[[UITextView alloc] initWithFrame:CGRectMake(0, 140, 320, 200-140)];
...
-(void)pullUpButtonClick:(UIButton *)button
{
    ...
    if (frame.origin.y==70) {
        ...
        frame.origin.y=70;
        detailFrame.size.height=screenHeight-140;
